...
from .ajaxtyping import Wlan, L2Rule, Mesh, Ap, _ClientStatsLevel1, _ApStatsLevel1
from .ruckusonetyping import AccessControlPolicyDict, AccessControlProfileDict
from .utility import normalize_mac_upper
from .const import (
...
class RuckusOneAjaxApi(RuckusAjaxApi):
    ...
    async def get_blocked_client_macs(self) -> list[L2Rule]:
        ...
        return [] if not acl else [L2Rule({"mac": mac}) for mac in acl["macAddresses"]]
    ...
    async def _do_remove_block_policy_from_networks(
        self, blocklist: AccessControlPolicyDict
    ) -> None:
        blocklist_id = blocklist["id"]
        update_tasks = []
        for acl_profile in await self._query_acl_profiles():
            if (
                "l2AclPolicy" in acl_profile
                and acl_profile["l2AclPolicy"]["id"] == blocklist_id
            ):
                update_tasks.append(
                    self.__session.delete(
                        f"accessControlProfiles/{acl_profile['id']}/l2AclPolicies/{blocklist_id}"
                    )
                )
        if "wifiNetworkIds" in blocklist:
            for blocklist_id in blocklist["wifiNetworkIds"]:
                update_tasks.append(
                    self.__session.delete(
                        f"wifiNetworks/{blocklist_id}/l2AclPolicies/{blocklist_id}"
                    )
                )
        if update_tasks:
            await asyncio.gather(*update_tasks)
    ...
    async def _do_apply_block_policy_to_networks(
        self, blocklist: AccessControlPolicyDict
    ) -> None:
        blocklist_id = blocklist["id"]
        all_l2_policies, all_wlans = await asyncio.gather(
            self._query_l2_policies(), self.get_wlans()
        )
        update_tasks = []
        ...
        if uncovered_wlan_ids:
            for wlan_id in uncovered_wlan_ids:
                task = self.__session.put(
                    f"wifiNetworks/{wlan_id}/l2AclPolicies/{blocklist_id}"
                )
    ...
    async def _query_acl_profiles(
        self,
        filters: dict | None = None,
        l2_policies: list[AccessControlPolicyDict] | None = None,
    ) -> list[AccessControlProfileDict]:
        query_params = {"filters": filters} if filters else {}
        profiles = await self.__session.query(
            "accessControlProfiles/query", query_params
        )
        if profiles:
            await self._enrich_acl_profiles_with_l2_policies(profiles, l2_policies)
        return profiles

    async def _enrich_acl_profiles_with_l2_policies(
        self,
        profiles: list[AccessControlProfileDict],
        policies: list[AccessControlPolicyDict] | None,
    ) -> None:
        if policies is None:
            policies_to_fetch = {
                profile_id for p in profiles if (profile_id := p.get("l2AclPolicyId"))
            }
        ...
        for profile in profiles:
            if profile_id := profile.pop("l2AclPolicyId", None):
                assert profile_id in policy_map
                profile["l2AclPolicy"] = policy_map[profile_id]
    ...
    async def _query_l2_policies(
        ...
        if policies:
            ...
            policies_with_macs = await asyncio.gather(
                *[self._get_l2_policy(policy["id"]) for policy in policies]
            )
            policies = [
    ...
    async def _get_l2_policy(self, policy_id: str) -> AccessControlPolicyDict:
        return await self.__session.get(f"l2AclPolicies/{policy_id}")

    async def _do_update_l2_policy(self, policy: AccessControlPolicyDict) -> None:
        json = cast(dict, deepcopy(policy))
        policy_id = json["id"]
        del json["id"]
        await self.__session.put(f"l2AclPolicies/{policy_id}", json)
