...
class RuckusAjaxApi(RuckusConfigurationApi):
    ...
    async def get_ap_events(self, *ap_macs, limit: int = 300) -> list[Event]:
        ...
        return [xevent async for xevent in self.cmdstat_piecewise("eventd", "xevent", filters={"ap": list(normalize_mac_lower(mac) for mac in ap_macs) if ap_macs else "*"}, limit=limit)]
